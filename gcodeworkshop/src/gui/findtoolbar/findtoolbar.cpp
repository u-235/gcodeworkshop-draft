...
GUI::FindToolBar::FindToolBar(GCodeWorkShop* app, QWidget* parent) :
	QToolBar(parent),
	m_app(app),
	m_actions(new GUI::FindToolBarActions(app)),
	m_returnReaction(NO),
...
void GUI::FindToolBar::highlight()
{
...
void GUI::FindToolBar::activate(bool replace)
{
	Q_UNUSED(replace)
	QString lookFor;
	GCoderDocument* gdoc = m_app->activeGCoderDocument();
...
void GUI::FindToolBar::deActivate()
{
	GCoderDocument* gdoc = m_app->activeGCoderDocument();
...
void GUI::FindToolBar::setReturnReaction(ReturnReaction reaction)
{
	m_returnReaction = reaction;
}

GUI::FindToolBar::ReturnReaction GUI::FindToolBar::returnReaction() const
{
	return m_returnReaction;
}

bool GUI::FindToolBar::findInDocument(GCoderDocument* gdoc, bool backward)
{
...
bool GUI::FindToolBar::replaceInDocument(GCoderDocument* gdoc, bool backward, bool all)
{
	if (!gdoc || gdoc->isReadOnly()) {
		return false;
	}
...
void GUI::FindToolBar::hideEvent(QHideEvent* event)
{
	deActivate();
	QWidget::hideEvent(event);
}

void GUI::FindToolBar::closeEvent(QCloseEvent* event)
{
	deActivate();
	QWidget::closeEvent(event);
}

void GUI::FindToolBar::findReturnPressed()
{
	GCoderDocument* gdoc = m_app->activeGCoderDocument();
...
void GUI::FindToolBar::findTextChanged()
{
...
void GUI::FindToolBar::loadIcons()
{
}

void GUI::FindToolBar::loadSettings(QSettings* cfg)
{
...
void GUI::FindToolBar::saveSettings(QSettings* cfg) const
{
	QString rr;
...
GUI::Actions::ActionKit* GUI::FindToolBar::actions() const
{
	return m_actions;
}

bool GUI::FindToolBar::isWholeWords() const
{
	return m_wholeWords->isChecked();
}

bool GUI::FindToolBar::isIgnoreCase() const
{
	return m_ignoreCase->isChecked();
}

bool GUI::FindToolBar::isIgnoreComments() const
{
	return m_ignoreComments->isChecked();
}

QString GUI::FindToolBar::textForFind() const
{
	return m_findEdit->text();
}

void GUI::FindToolBar::setTextForFind(const QString& text)
{
	if (text.size() < 32) {
...
QString GUI::FindToolBar::textForReplace() const
{
